...
public class Stream_03_Test {
	...
	public void test_map_collect_joining() throws Exception {
		...
		// par le caractère "|"
		String result = customers.stream().map(Customer::getFirstname).sorted().collect(joining("|"));
	...
	public void test_flatMap() throws Exception {
		...
		List<Pizza> result = orders.stream().flatMap(o -> o.getPizzas().stream()).collect(toList());
	...
	public void test_flatMap_distinct() throws Exception {
		...
		List<Pizza> result = orders.stream().flatMap(o -> o.getPizzas().stream()).distinct().collect(toList());
